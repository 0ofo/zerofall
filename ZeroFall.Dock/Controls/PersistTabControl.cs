...
public class PersistTabControl : TabControl
{
    ...
    /// <summary>单 Tab 时隐藏标签栏（区域完全交给内容管控），多 Tab 时显示。</summary>
    private void UpdateTabStripVisibility()
    {
        if (_tabStripHost is null)
            return;
        var count = 0;
        foreach (var _ in ResolveItemsEnumerable())
        {
            count++;
            if (count > 1) break;
        }
        _tabStripHost.IsVisible = count > 1;
    }
    ...
    private void SyncNonReloadableOverlay()
    {
        ...
        foreach (var item in ResolveItemsEnumerable())
        {
            ...
            var isSelected = ReferenceEquals(item, selected);
            content.IsVisible = isSelected;
            content.IsHitTestVisible = isSelected;
    ...
    private static void NotifyDataTableShown(Control host)
    {
        ...
        var table = host as DataTableView
                    ?? host.GetVisualDescendants().OfType<DataTableView>().FirstOrDefault();
        table?.NotifyTabActivated();

        foreach (var nested in host.GetVisualDescendants().OfType<ITableGridHost>())
            nested.NotifyTabActivated();
    }
