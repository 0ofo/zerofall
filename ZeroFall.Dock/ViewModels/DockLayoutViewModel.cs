...
public partial class DockLayoutViewModel : ViewModelBase
{
    ...
    {
        if (selectedTab?.LinkedTabIds is not { } links) return;

        foreach (var (region, tabId) in links)
        {
            if (region == DockPosition.Content) continue;
            var panel = GetPanelForRegion(region);
            if (panel == null) continue;
            var tab = panel.Tabs.FirstOrDefault(t => t.Id == tabId)
                      ?? TryAddOnDemandTabShell(panel, tabId);
            if (tab != null && panel.SelectedTab != tab)
                panel.SelectedTab = tab;
        }
    }
