...
public sealed class UiLayoutService : IUiLayoutService
{
    ...
    public async Task<UiLayoutSnapshot> GetSnapshotAsync()
    {
        var menu = CaptureMenu();
        if (_dock == null)
            return new UiLayoutSnapshot(menu, [], [], [], []);
        ...
        return await UiThreadBridge.InvokeAsync(() =>
            _dock.CaptureLayoutSnapshot(menu, _extraProviders)).ConfigureAwait(false);
    }
    ...
    public string GetLayoutJson(UiLayoutQuery query = default)
    {
        ...
        return SerializeLayoutJson(snapshot, query, tabId);
    }

    public async Task<string> GetLayoutJsonAsync(UiLayoutQuery query = default)
    {
        if (query.Scope == UiLayoutScope.Tab && string.IsNullOrWhiteSpace(query.TabId))
            return ToolResultJson.Error("Tab Id 不能为空");
        ...
        var tabId = query.TabId?.Trim() ?? string.Empty;
        return await Task.Run(() => SerializeLayoutJson(snapshot, query, tabId)).ConfigureAwait(false);
    }

    private static string SerializeLayoutJson(UiLayoutSnapshot snapshot, UiLayoutQuery query, string tabId) =>
        query.Scope switch
        {
    ...
    public string SwitchTab(string tabId)
    {
        ...
        return SwitchTabAsync(tabId).GetAwaiter().GetResult();
    }

    public Task<string> SwitchTabAsync(string tabId)
    {
        if (string.IsNullOrWhiteSpace(tabId))
            return Task.FromResult(ToolResultJson.Error("tabId 不能为空"));

        tabId = tabId.Trim();
        if (_dock == null)
            return Task.FromResult(ToolResultJson.Error("UI 布局尚未就绪"));

        return UiThreadBridge.InvokeAsync(() => SwitchTabCore(tabId));
    }
    ...
    private static string SerializeTabFocus(UiLayoutSnapshot snapshot, string tabId)
    {
        ...
        if (focus == null)
        {
            return ToolResultJson.Error(
                $"未找到 Tab「{tabId}」。请先用 ui_context 查看各区域 Tab 的 id，或确认 Tab 已打开。");
        }
    ...
    private string SwitchTabCore(string tabId)
    {
        ...
        {
            return ToolResultJson.Error(
                $"未找到 Tab「{tabId}」。请先调用 ui_context 查看 sidebar/content/bottom/right 各区域已打开 Tab 的 id。");
        }
