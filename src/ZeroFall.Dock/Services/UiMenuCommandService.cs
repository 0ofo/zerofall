...
public sealed class UiMenuCommandService : IUiMenuCommandService
{
    ...
    public string Execute(string commandId, UiMenuCommandArgs? args = null)
    {
        ...
        return ExecuteAsync(commandId, args).GetAwaiter().GetResult();
    }

    public Task<string> ExecuteAsync(string commandId, UiMenuCommandArgs? args = null)
    {
        if (string.IsNullOrWhiteSpace(commandId))
            return Task.FromResult(ToolResultJson.Error("commandId 不能为空"));

        commandId = commandId.Trim();
        args ??= new UiMenuCommandArgs();
        return UiThreadBridge.InvokeAsync(() => ExecuteCore(commandId, args));
    }
    ...
    private string ExecuteCore(string commandId, UiMenuCommandArgs args)
    {
        switch (commandId)
        {
            ...
            default:
                return ToolResultJson.Error(
                    $"未知 commandId「{commandId}」。请 ui_context scope=menu 查看可用项。");
        }
    }
