...
internal class CsvDataContentFactory : IContentFactory
{
    ...
    public object? CreateContent(ContentFactoryContext context)
    {
        ...
        try
        {
            ...
            var eventBus = _sp.GetService<IEventBus>();
            return new FilePreviewView(context.FilePath, null, dataTable, eventBus);
        }
        catch
        {
            return null;
...
internal class TextFileContentFactory : IContentFactory
{
    ...
    public object? CreateContent(ContentFactoryContext context)
    {
        ...
        {
            var eventBus = _sp.GetService<IEventBus>();
            return new FilePreviewView(context.FilePath, null, null, eventBus);
        }
        catch
        {
            return null;
