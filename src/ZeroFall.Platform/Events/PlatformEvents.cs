...
/// <summary>未打开项目时从欢迎页进入 Dock 工作区（终端/侦察等）。</summary>
public record WelcomeQuickAccessRequestedEvent;
...
/// <summary>在 Content 区用文本编辑器打开工作区文件（菜单/快捷键等用户操作）。</summary>
/// <summary>工作区文件被外部进程创建、修改或删除（侧边栏 FileSystemWatcher 防抖后发布）。</summary>
public record WorkspaceFileChangedEvent(string FilePath, bool Deleted = false);
...
/// <summary>Dock Tab 关闭前发布；<see cref="TabCloseRequestedEvent.Cancel"/> 为 true 时中止关闭。</summary>
public sealed class TabCloseRequestedEvent(Registries.DockTabItemViewModel tab)
{
    public Registries.DockTabItemViewModel Tab { get; } = tab;
    public bool Cancel { get; set; }
}
public record DockTabSelectedEvent(Registries.DockPosition Region, Registries.DockTabItemViewModel? Tab);
...
/// <summary>在指定 Dock 区域追加或激活同 Id 标签；默认选中新标签，批量同步时可关闭选中。</summary>
public record AddDockTabEvent(Registries.DockPosition Region, Registries.DockTabItemViewModel Tab, bool Select = true);
public record RemoveDockTabEvent(Registries.DockPosition Region, string TabId);
public record UpdateDockTabTitleEvent(Registries.DockPosition Region, string TabId, string Title);
