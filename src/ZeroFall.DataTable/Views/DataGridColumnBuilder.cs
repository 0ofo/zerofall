...
public static class DataGridColumnBuilder
{
    ...
    /// <summary>URL 列单元格点击时打开链接；返回 true 表示已处理。</summary>
    public static bool TryHandleUrlCellPointerPress(DataTableViewModel vm, DataGridCellPointerPressedEventArgs e)
    {
        if (vm.DisableUrlColumns) return false;
        if (e.Row is null) return false;
        if (e.Column?.Header is not string header || !IsUrlHeader(header)) return false;
        if (e.Row.DataContext is not DataRowViewModel row) return false;
        ...
        if (idx < 0 || idx >= row.Values.Count) return false;
